...
rect_wh _rect2D(rect_xywhf* const * v, int n, int max_ws, int max_hs, vector<rect_xywhf*>& succ, vector<rect_xywhf*>& unsucc) {
	node root;
	...
	rect_wh min_bin = rect_wh(max_ws, max_hs);
...
bool pack(rect_xywhf* const * v, int n, int max_ws, int max_hs,vector<bin>& bins) {
	rect_wh _rect(max_ws, max_hs);
	...
	while(true) {
		...
		b->size = _rect2D(&((*p[0])[0]), p[0]->size(), max_ws, max_hs, b->rects, *p[1]);
		p[0]->clear();
...
rect_xywh::rect_xywh(int x, int y, int w, int h, int index) : x(x), y(y), rect_wh(w, h), index(index){}
...
rect_xywhf::rect_xywhf(int x, int y, int width, int height, int index) : rect_xywh(x, y, width, height, index), flipped(false) {}
