...
struct rect_xywh : public rect_wh {
	...
	rect_xywh(int x, int y, int width, int height, int index);
	operator rect_ltrb();

	int x, y, index, r() const, b() const;
	void r(int), b(int);
};

struct rect_xywhf : public rect_xywh {
	rect_xywhf(const rect_ltrb&);
	rect_xywhf(int x, int y, int width, int height, int index);
	rect_xywhf();
	void flip();
	bool flipped;
};
...
bool pack(rect_xywhf* const * v, int n, int max_ws, int max_hs, std::vector<bin>& bins);
